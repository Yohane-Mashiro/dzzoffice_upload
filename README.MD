...
1. 组件：UEditor 后端（文件上传）
   - 问题与类型：
     - 任意文件上传到 Web 根（getFilePath 使用 `$_SERVER['DOCUMENT_ROOT']` 拼接 `pathFormat`），config.json 的 `fileAllowFiles` 默认包含 `.html`、`.js`、`.css` 等可执行前端类型 → 可直接写入可被访问并执行的页面/脚本（持久 XSS）。
